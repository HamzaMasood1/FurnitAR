...
public class Database extends SQLiteAssetHelper {
    ...
    public List<Order> getCarts() {
        ...
        if (cursor.moveToNext()) {
            do {
                result.add(new Order(cursor.getInt(cursor.getColumnIndex("userID")),
    ...
    public void addToCart(Order order) {
        try {
            ...
            int quantity = 0;
            Cursor c = db.rawQuery(query, new String[]{String.valueOf(order.getId())});
            while (c.moveToNext()) {
    ...
    public Pair<String, Integer> removeFromCart(Order order) {
        ...
        Cursor c = qb.query(db, sqlselect, "id = ?", new String[]{String.valueOf(order.getId())}, null, null, null);
